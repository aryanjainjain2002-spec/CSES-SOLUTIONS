...
void dfs1(int node,vector<vector<int>> & adj,vector<int>&res,vector<int>&commonLcas,int parent){

    for(int child : adj[node]){
        if(child == parent)continue;

        dfs1(child,adj,res,commonLcas,node);
        res[node] += res[child] - commonLcas[child];
    }
}
